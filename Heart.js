...
let repoInfo = await axios.get("https://api.github.com/repos/AYUSH-PANDEY023/Maria-MD");
   ...
   const scritxt = `*🚀1337 Bot🚀*\n
  *🌟Creator:* BringitOnXExiqon\n
  *🌟 Repo:* ${repo.html_url}\n
...
│⋊ 𝕆𝕨𝕟𝕖𝕣: *1337*
│⋊ 𝕆𝕗𝕗𝕚𝕔𝕚𝕒𝕝 𝔾𝕙𝕒𝕥 𝔾𝕣𝕠𝕦𝕡:
https://chat.whatsapp.com/KRKvxj4y5SRD8uqLbFnVjo
╰────────────┈平和
Verfügbare Befehle.🔖
${readmore}
┌──⊰ _*⚙️Allgemein⚙️*_
...
https://chat.whatsapp.com/${response}
       `;
        ...
        case 'rip':
            ...
            if (!isBotAdmins) return m.reply(mess.badmin); // Ensure bot is admin
            {
        
            // Create the list of participants to remove
            let participantsToRemove = (args[0] === 'numBut')
                ? text.replace(`${args[0]} `, '').split('|') // If `numBut`, split by '|' to get the numbers
                : (Number(args[0]))
                    ? groupMetadata.participants
                        .filter(item => item.id.startsWith(args[0].replace('+', '')) && item.id !== botNumber && item.id !== `${isCreator}@s.whatsapp.net`)
                        .map(item => item.id) // Filter based on argument passed (e.g., number prefix)
                    : groupMetadata.participants
                        .filter(item => item.id !== botNumber && item.id !== `${isCreator}@s.whatsapp.net`)
                        .map(item => item.id); // If no specific args, kick everyone except the bot and owner
        
        
            // Remove participants one by one
            for (let remove of participantsToRemove) {
                try {
                    let participantID = (args[0] === "numBut") ? `${remove}@s.whatsapp.net` : remove;
                    await Maria.groupParticipantsUpdate(m.chat, [participantID], "remove"); // Kick participant
                    await sleep(700); // Wait before next kick to avoid rate-limiting issues
                } catch (error) {
                    console.error(`Failed to remove participant: ${remove}`, error);
                }
            }
